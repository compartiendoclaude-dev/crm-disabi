...
import { fmtUSD, estaVencida } from '@/lib/utils'
...
interface CxcRow {
  id: string; fecha_venta: string; fecha_vence?: string
  monto_total: number; monto_pendiente: number; estado: string
                    ...
                    <div key={cxc.id} style={{ marginBottom: 14, border: '1px solid var(--bdr)',
                      ...
                      <div style={{ padding: '10px 14px', background: 'var(--surf2)',
                        ...
                        <div>
                          ...
                          <EstBadge estado={cxc.estado === 'Pagado' ? cxc.estado : estaVencida(cxc) ? 'Vencido' : cxc.estado} />
                        </div>
                        <div style={{ display: 'flex', gap: 16, fontSize: 12 }}>
                          <span>Emisión: <strong>{cxc.fecha_venta}</strong></span>
                          {cxc.fecha_vence && <span>Vence: <strong>{cxc.fecha_vence}</strong></span>}
                          <span>Monto: <strong style={{ fontFamily: 'monospace' }}>{fmtUSD(cxc.monto_total)}</strong></span>
                          <span>Saldo: <strong style={{ fontFamily: 'monospace', color: cxc.monto_pendiente > 0 ? '#d97706' : '#16a34a' }}>{fmtUSD(cxc.monto_pendiente)}</strong></span>
                        </div>
                      </div>
                      {/* Abonos */}
