...
import { fmtUSD, monthLabel, estaVencida } from '@/lib/utils'
...
// Columnas reales: disabi_cxc no tiene numero/fecha_emision/monto/saldo — son
// fecha_venta, monto_total, monto_pendiente. disabi_cpp sí tiene fecha_emision,
// y numero_doc en vez de numero.
interface CXCRow { id: string; cliente: string; fecha_venta: string; fecha_vence?: string; monto_total: number; monto_pendiente: number; estado: string }
interface CPPRow { id: string; numero_doc?: string; proveedor: string; fecha_emision: string; fecha_vence?: string; monto_total: number; monto_pendiente: number; estado: string }
...
export default function ReportesModule({ ventas, gastos, costosFijos, cxc, cpp, productos, clientes, ppPendientes, finMes, hoy, mesActual, mesInicio, mesFin }: Props) {
  ...
  const renderPreview = (key: ReporteKey) => {
    switch (key) {
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 16 }}>
              {[
                ...
                { l: 'CxC pendiente', v: fmtUSD(cxc.filter(x => x.estado !== 'Pagado').reduce((a, x) => a + x.monto_pendiente, 0)), c: 'var(--amber)' },
              ].map(k => (
      ...
      case 'cxc':
        return (
          <div>
            ...
            <table className="tbl">
              <thead><tr><th>#</th><th>Cliente</th><th>Venta</th><th>Vence</th><th>Monto orig.</th><th>Saldo</th><th>Estado</th></tr></thead>
              <tbody>
                {cxc.map(x => (
                  <tr key={x.id}>
                    <td className="mono" style={{ fontSize: 10 }}>{x.id.slice(0, 8)}</td>
                    <td style={{ fontWeight: 600 }}>{x.cliente}</td>
                    <td className="mono" style={{ fontSize: 11 }}>{x.fecha_venta}</td>
                    <td className="mono" style={{ fontSize: 11 }}>{x.fecha_vence ?? '–'}</td>
                    <td className="mono">{fmtUSD(x.monto_total)}</td>
                    <td className="mono" style={{ fontWeight: 700, color: x.monto_pendiente <= 0 ? 'var(--green)' : 'var(--amber)' }}>{fmtUSD(x.monto_pendiente)}</td>
                    <td><span className={`badge ${x.estado === 'Pagado' ? 'badge-green' : estaVencida(x) ? 'badge-red' : 'badge-amber'}`}>{x.estado === 'Pagado' ? x.estado : estaVencida(x) ? 'Vencido' : x.estado}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div>
            ...
            <table className="tbl">
              ...
              <tbody>
                {cpp.map(x => (
                  <tr key={x.id}>
                    <td className="mono" style={{ fontSize: 10 }}>{x.numero_doc ?? x.id.slice(0, 8)}</td>
                    <td style={{ fontWeight: 600 }}>{x.proveedor}</td>
                    <td className="mono" style={{ fontSize: 11 }}>{x.fecha_emision}</td>
                    <td className="mono" style={{ fontSize: 11 }}>{x.fecha_vence ?? '–'}</td>
                    <td className="mono">{fmtUSD(x.monto_total)}</td>
                    <td className="mono" style={{ fontWeight: 700, color: x.monto_pendiente <= 0 ? 'var(--green)' : 'var(--red)' }}>{fmtUSD(x.monto_pendiente)}</td>
                    <td><span className={`badge ${x.estado === 'Pagado' ? 'badge-green' : estaVencida(x) ? 'badge-red' : 'badge-amber'}`}>{x.estado === 'Pagado' ? x.estado : estaVencida(x) ? 'Vencido' : x.estado}</span></td>
                  </tr>
                ))}
              </tbody>
      ...
      case 'indicadores': {
        const cxcTotal       = cxc.filter(x => x.estado !== 'Pagado').reduce((a, x) => a + x.monto_pendiente, 0)
        const cppTotal       = cpp.filter(x => x.estado !== 'Pagado').reduce((a, x) => a + x.monto_pendiente, 0)
                ...
                <div className="card">
                  ...
                  {cpp.filter(x => x.estado !== 'Pagado').slice(0, 10).map(x => (
                    <div key={x.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, padding: '4px 0', borderBottom: '1px solid var(--bdr)' }}>
                      ...
                      <span className="mono" style={{ color: 'var(--red)', fontWeight: 700 }}>{fmtUSD(x.monto_pendiente)}</span>
                    </div>
                  ))}
                </div>
              </div>
