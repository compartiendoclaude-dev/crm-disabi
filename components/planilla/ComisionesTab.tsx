...
interface ResultadoComparacion {
  hoja_usada: string
  periodo: string
  comision_total_excel: number
  comision_total_sistema: number
  diferencia_total: number
  comparacion: LineaComparacion[]
}
...
export default function ComisionesTab({ empleados, comisiones: initialComisiones, mesActual }: Props) {
  ...
  // Comparar contra el Excel maestro (auditoría — no reemplaza el cálculo automático)
  const [compFile,     setCompFile]     = useState<File | null>(null)
  const [compPeriodo,  setCompPeriodo]  = useState(mesActual)
  const [comparando,   setComparando]   = useState(false)
  const [compResult,   setCompResult]   = useState<ResultadoComparacion | null>(null)
  ...
  async function handleComparar() {
    if (!compFile) return setCompError('Selecciona el archivo Excel de comisiones (.xlsx)')
    setComparando(true); setCompError(''); setCompResult(null)
    try {
      const fd = new FormData()
      fd.append('archivo', compFile)
      fd.append('periodo', compPeriodo)
      const res = await fetch('/api/comisiones/comparar', { method: 'POST', body: fd })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error)
      setCompResult(data)
    } catch (e: unknown) {
      setCompError(e instanceof Error ? e.message : 'Error al comparar')
    ...
    <div style={{ padding: '0 0 20px' }}>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--txt)', marginBottom: 4 }}>
          📊 Comparar con Excel de Comisiones
        </div>
        <div style={{ fontSize: 11, color: 'var(--txt3)', marginBottom: 12 }}>
          Sube tu Excel de comisiones para comparar sus totales por categoría contra lo que el sistema calcula
          a partir de las ventas registradas. Es una auditoría — no reemplaza el cálculo automático de arriba.
        </div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end', marginBottom: 12 }}>
          <div className="field" style={{ flex: '1 1 260px' }}>
            <label>Archivo Excel (.xlsx)</label>
            <input type="file" accept=".xlsx,.xls,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
              onChange={e => { setCompFile(e.target.files?.[0] ?? null); setCompResult(null); setCompError('') }} />
          </div>
          <div className="field" style={{ flex: '0 0 140px' }}>
            <label>Período</label>
            <input type="month" value={compPeriodo} onChange={e => { setCompPeriodo(e.target.value); setCompResult(null) }} />
          </div>
          <button className="btn btn-primary" onClick={handleComparar} disabled={comparando || !compFile}
            style={{ marginBottom: 2 }}>
            {comparando ? '⏳ Comparando...' : '🔍 Comparar'}
          </button>
        </div>

        {compError && <div style={{ background: 'rgba(220,38,38,.1)', border: '1px solid rgba(220,38,38,.3)', borderRadius: 'var(--r)', padding: '8px 12px', fontSize: 12, color: 'var(--red)', marginBottom: 10 }}>{compError}</div>}

        {compResult && (
          <div style={{ borderTop: '1px solid var(--bdr)', paddingTop: 14 }}>
            <div style={{ fontSize: 11, color: 'var(--txt3)', marginBottom: 10 }}>
              Comparando contra la hoja <strong>&ldquo;{compResult.hoja_usada}&rdquo;</strong> del archivo, período {compResult.periodo}.
            </div>
            ...
            <div style={{ overflowX: 'auto', marginBottom: 12 }}>
              <table className="tbl">
                <thead><tr>
                  <th>Categoría</th>
                  <th>Cant. Excel</th><th>Comisión Excel</th>
                  <th>Cant. Sistema</th><th>Comisión Sistema</th>
                  <th>Diferencia</th>
                </tr></thead>
                <tbody>
                  {compResult.comparacion.map(l => {
                    const coincide = Math.abs(l.diferencia) < 0.01
                    return (
                      <tr key={l.categoria}>
                        <td style={{ fontWeight: 600 }}>
                          {CATEGORIA_ICONS[l.categoria] ?? '📦'} {l.categoria}
                          {!coincide && <span style={{ marginLeft: 6 }} title="No coincide">⚠️</span>}
                        </td>
                        <td className="mono" style={{ textAlign: 'center' }}>{l.cantidad_excel}</td>
                        <td className="mono">{fmtUSD(l.comision_excel)}</td>
                        <td className="mono" style={{ textAlign: 'center' }}>{l.cantidad_sistema}</td>
                        <td className="mono">{fmtUSD(l.comision_sistema)}</td>
                        <td className="mono" style={{ fontWeight: 700, color: coincide ? 'var(--green)' : 'var(--red)' }}>
                          {l.diferencia > 0 ? '+' : ''}{fmtUSD(l.diferencia)}
                        </td>
                      </tr>
                    )
                  })}
                ...
                <tfoot>
                  <tr style={{ borderTop: '2px solid var(--bdr)', fontWeight: 800 }}>
                    ...
                    <td className="mono">{fmtUSD(compResult.comision_total_excel)}</td>
                    <td></td>
                    <td className="mono">{fmtUSD(compResult.comision_total_sistema)}</td>
                    <td className="mono" style={{ color: Math.abs(compResult.diferencia_total) < 0.01 ? 'var(--green)' : 'var(--red)' }}>
                      {compResult.diferencia_total > 0 ? '+' : ''}{fmtUSD(compResult.diferencia_total)}
                    </td>
                  </tr>
                </tfoot>
              </table>
            ...
            {Math.abs(compResult.diferencia_total) < 0.01 ? (
              <div style={{ background: 'rgba(22,163,74,.1)', border: '1px solid rgba(22,163,74,.3)', borderRadius: 'var(--r)', padding: '8px 12px', fontSize: 12, color: 'var(--green)' }}>
                ✅ El total coincide con el Excel.
              </div>
            ) : (
              <div style={{ background: 'rgba(217,119,6,.1)', border: '1px solid rgba(217,119,6,.3)', borderRadius: 'var(--r)', padding: '8px 12px', fontSize: 12, color: 'var(--amber)' }}>
                ⚠️ Hay diferencia entre el Excel y el sistema. Puede deberse a: productos sin Categoría de Comisión asignada en Inventario,
                ventas sin registrar en Ventas todavía, o devoluciones/estado de cobro distinto entre ambas fuentes.
              </div>
            )}
          </div>
        )}
