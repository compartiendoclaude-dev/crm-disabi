...
interface GastoLocal { id: string; fecha: string; descripcion: string; monto: number; categoria?: string; proveedor?: string; factura?: string; fecha_vence_pago?: string | null }
...
// ─── Modal: Importar JSON de compras (DTE recibidos de proveedores) ───────────
// Sin IA — parsea el JSON estructurado del MH directamente. Costo: $0.00.
function ImportarJsonComprasModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  type ItemPreview = { descripcion: string; cantidad: number; match: boolean }
  type DocPreview = {
    ...
    items?: ItemPreview[]; ya_existe?: boolean
    tipo_egreso_sugerido?: 'compra_local' | 'operativo'
  }
  type Resultado = { nombre: string; ok: boolean; numero_control?: string; error?: string; items_creados?: number; items_sin_match?: number; tipo_egreso?: string }

  const [step, setStep] = React.useState<'subir' | 'revisar' | 'listo'>('subir')
  const [procesando, setProcesando] = React.useState(false)
  const [error, setError] = React.useState('')
  const [archivosCache, setArchivosCache] = React.useState<{ nombre: string; json: unknown }[]>([])
  const [docsPreview, setDocsPreview] = React.useState<DocPreview[]>([])
  const [tipoPorProveedor, setTipoPorProveedor] = React.useState<Record<string, 'compra_local' | 'operativo'>>({})
  const [guardando, setGuardando] = React.useState(false)
  const [resultado, setResultado] = React.useState<{ exitosos: number; fallidos: number; resultados: Resultado[] } | null>(null)

  async function handleArchivos(files: FileList) {
    setProcesando(true); setError('')
    try {
      const documentos = await Promise.all(
        Array.from(files).map(file => new Promise<{ nombre: string; json: unknown }>((res, rej) => {
          const reader = new FileReader()
          reader.onload = () => {
            try {
              // Algunos sistemas de facturación (ej: CORPAN/San Martin) generan JSON
              // con BOM (byte order mark) al inicio, que rompe JSON.parse — se limpia aquí.
              const texto = (reader.result as string).replace(/^\uFEFF/, '')
              res({ nombre: file.name, json: JSON.parse(texto) })
            }
            catch { rej(new Error(`"${file.name}" no es un JSON válido`)) }
          }
          reader.onerror = rej
          reader.readAsText(file)
        }))
      )
      setArchivosCache(documentos)

      const res = await fetch('/api/compras-dte', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'previsualizar_compras_dte', documentos }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error ?? 'Error al leer los documentos')

      setDocsPreview(data.documentos)

      // Inicializar el tipo por proveedor con la sugerencia automática
      const inicial: Record<string, 'compra_local' | 'operativo'> = {}
      for (const [prov, info] of Object.entries(data.porProveedor as Record<string, { tipo_egreso_sugerido: 'compra_local' | 'operativo' }>)) {
        inicial[prov] = info.tipo_egreso_sugerido
      }
      setTipoPorProveedor(inicial)
      setStep('revisar')
    } catch (e: unknown) {
  ...
  // Agrupar preview por proveedor para la pantalla de revisión
  const proveedoresUnicos = Array.from(new Set(docsPreview.filter(d => d.ok && d.proveedor).map(d => d.proveedor as string)))
  const docsConError = docsPreview.filter(d => !d.ok)
  const docsDuplicados = docsPreview.filter(d => d.ok && d.ya_existe)
  const docsAImportar = docsPreview.filter(d => d.ok && !d.ya_existe)

  async function confirmarImportacion() {
    setGuardando(true); setError('')
    try {
      // Solo reenviar los archivos que sí se van a importar (no duplicados, no con error)
      const nombresAImportar = new Set(docsAImportar.map(d => d.nombre))
      const documentosFiltrados = archivosCache.filter(a => nombresAImportar.has(a.nombre))

      const res = await fetch('/api/compras-dte', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'importar_compras_dte',
          documentos: documentosFiltrados,
          tipoEgresoPorProveedor: tipoPorProveedor,
        }),
      })
      const data = await res.json()
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" style={{ maxWidth: 760 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
          <h3 style={{ fontWeight: 800, fontSize: 15 }}>🧾 Importar JSON de compras (DTE proveedores)</h3>
        ...
        {step === 'subir' && (
          <div style={{ textAlign: 'center', padding: '36px 20px' }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--txt3)', marginBottom: 6 }}>
              {procesando ? 'Leyendo documentos...' : 'Sube uno o varios archivos JSON de facturas de compra'}
            </p>
            <p style={{ fontSize: 11, color: 'var(--green)', marginBottom: 18, fontWeight: 600 }}>
              ✓ Lectura estructurada — sin IA, sin costo
            </p>
            <label style={{
              display: 'inline-flex', alignItems: 'center', gap: 8,
              padding: '10px 20px', borderRadius: 'var(--r)',
              ...
              📎 Seleccionar JSON (uno o varios)
              <input
                type="file" accept="application/json,.json" multiple style={{ display: 'none' }}
                disabled={procesando}
                onChange={e => { if (e.target.files?.length) handleArchivos(e.target.files); e.target.value = '' }}
              />
            </label>
          </div>
        )}

        {/* PASO 2: Revisar — agrupado por proveedor, categoría editable */}
        {step === 'revisar' && (
          <>
            <div style={{ display: 'flex', gap: 16, marginBottom: 14, fontSize: 12 }}>
              <span>📄 {docsPreview.length} documentos leídos</span>
              {docsDuplicados.length > 0 && <span style={{ color: 'var(--amber)' }}>⚠️ {docsDuplicados.length} ya importados (se omitirán)</span>}
              {docsConError.length > 0 && <span style={{ color: 'var(--red)' }}>❌ {docsConError.length} con error</span>}
            </div>

            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--txt3)', textTransform: 'uppercase', marginBottom: 8 }}>
              Asignar categoría por proveedor
            </div>
            <div style={{ maxHeight: 200, overflow: 'auto', border: '1px solid var(--bdr)', borderRadius: 'var(--r)', marginBottom: 16 }}>
              {proveedoresUnicos.map(prov => {
                const docsDeEste = docsPreview.filter(d => d.proveedor === prov && d.ok && !d.ya_existe)
                const totalProv = docsDeEste.reduce((a, d) => a + (d.total ?? 0), 0)
                if (docsDeEste.length === 0) return null
                return (
                  <div key={prov} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderBottom: '1px solid var(--bdr)' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{prov}</div>
                      <div style={{ fontSize: 10, color: 'var(--txt3)' }}>{docsDeEste.length} doc(s) · {fmtUSD(totalProv)}</div>
                    </div>
                    <select
                      value={tipoPorProveedor[prov] ?? 'operativo'}
                      onChange={e => setTipoPorProveedor(prev => ({ ...prev, [prov]: e.target.value as 'compra_local' | 'operativo' }))}
                      style={{ border: '1px solid var(--bdr)', borderRadius: 4, padding: '4px 8px', fontSize: 11, flexShrink: 0 }}
                    >
                      <option value="compra_local">Costo de Ventas</option>
            ...
            {docsConError.length > 0 && (
              <details style={{ marginBottom: 14 }}>
                <summary style={{ fontSize: 11, color: 'var(--red)', cursor: 'pointer' }}>Ver {docsConError.length} documentos con error ▸</summary>
                {docsConError.map((d, i) => (
                  <div key={i} style={{ fontSize: 11, color: 'var(--red)', padding: '4px 0' }}>{d.nombre}: {d.error}</div>
                ))}
              </details>
            )}

            <div style={{ display: 'flex', gap: 10, justifyContent: 'space-between' }}>
              <button className="btn btn-secondary" onClick={() => { setStep('subir'); setDocsPreview([]) }}>← Subir otros archivos</button>
              <button className="btn btn-primary" onClick={confirmarImportacion} disabled={guardando || docsAImportar.length === 0}>
                {guardando ? '⏳ Importando...' : `💾 Importar ${docsAImportar.length} documentos`}
              </button>
            </div>
          </>
        )}
        ...
        {step === 'listo' && resultado && (
          <div>
            <div style={{
              padding: '12px 16px', borderRadius: 'var(--r)', marginBottom: 14,
              background: resultado.fallidos === 0 ? 'rgba(22,163,74,.08)' : 'rgba(217,119,6,.08)',
              display: 'flex', gap: 16, fontSize: 13, alignItems: 'center',
            }}>
              <span>✅ <strong>{resultado.exitosos}</strong> importados</span>
              {resultado.fallidos > 0 && <span style={{ color: 'var(--red)' }}>❌ <strong>{resultado.fallidos}</strong> con error</span>}
            </div>
            ...
            <div style={{ maxHeight: 320, overflow: 'auto' }}>
              {resultado.resultados.map((r, i) => (
                <div key={i} style={{
                  ...
                  <span style={{ color: 'var(--txt2)', flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {r.nombre}
                  </span>
                  {r.ok ? (
                    <span style={{ fontSize: 11, color: '#16a34a', flexShrink: 0 }}>
                      ✓ {r.items_creados} items
                      {(r.items_sin_match ?? 0) > 0 && <span style={{ color: 'var(--amber)' }}> · {r.items_sin_match} sin match</span>}
                      {r.tipo_egreso === 'operativo' && <span style={{ color: 'var(--indigo)' }}> · Gasto operativo</span>}
                    </span>
                  ) : (
                    <span style={{ fontSize: 11, color: 'var(--red)', flexShrink: 0 }}>✗ {r.error}</span>
                  )}
                </div>
              ))}
            </div>
...
function CompraLocalModal({ edit, onClose, onSaved }: {
  ...
  const [fechaVence,setFechaVence]= useState(edit?.fecha_vence_pago ?? '')
  const [saving,    setSaving]    = useState(false)
  const [error,     setError]     = useState('')
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch('/api/compras', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'save_compra_local', editId: edit?.id, fecha, proveedor, descripcion: desc, monto, partida, numero_factura: factura, fecha_vence_pago: fechaVence || null }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error)
      onSaved()
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" style={{ maxWidth: 480 }}>
        ...
        <div className="grid-2" style={{ marginBottom: 12 }}>
          ...
          <div className="field" style={{ gridColumn: 'span 2' }}>
            <label>Fecha de vencimiento de pago (si es a crédito)</label>
            <input type="date" value={fechaVence ?? ''} onChange={e => setFechaVence(e.target.value)} />
            <div style={{ fontSize: 11, color: 'var(--txt3)', marginTop: 4 }}>
              Déjalo vacío si la compra ya está pagada. Si se llena, se crea/actualiza automáticamente una Cuenta por Pagar en Finanzas.
            </div>
          </div>
        </div>
...
export default function ComprasModule({ compras: initialCompras, gastosLocales: initialLocales, productos, kpis, proveedores, proveedoresTextoLibre, comprasSinVincular, proveedoresKpis }: Props) {
  ...
  const [showImportarJsonCompras, setShowImportarJsonCompras] = useState(false)
  ...
  return (
    <div style={{ padding: 20 }}>
        ...
        {tab === 'importaciones'
          ? (
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="btn btn-secondary" onClick={() => setShowImportarJsonCompras(true)}>🧾 Importar JSON de compras (DTE)</button>
                ...
                {locales.map(g => (
                  <tr key={g.id}>
                    ...
                    <td style={{ fontSize: 11, color: 'var(--txt3)' }}>
                      {g.factura ?? '–'}
                      {g.fecha_vence_pago && <div><span className="badge badge-amber" style={{ marginTop: 4 }}>Crédito → {g.fecha_vence_pago}</span></div>}
                    </td>
                    <td>
      ...
      {showImportarJsonCompras && <ImportarJsonComprasModal onClose={() => setShowImportarJsonCompras(false)} onSaved={() => { setShowImportarJsonCompras(false); reload() }} />}
