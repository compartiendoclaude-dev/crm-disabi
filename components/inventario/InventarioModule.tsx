...
import { CATEGORIAS_COMISION } from '@/lib/constants'
...
function ProductoModal({ edit, onClose, onSaved }: {
  ...
  const [catCom,   setCatCom]   = useState(edit?.categoria_comision ?? '')
    ...
    try {
      const res = await fetch('/api/inventario', {
        ...
        body: JSON.stringify({
          ...
          codigo, nombre, descripcion: desc, categoria: cat, categoria_comision: catCom || null, unidad,
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" style={{ maxWidth: 560 }}>
        ...
        <div className="grid-2" style={{ marginBottom: 12 }}>
          ...
          <div className="field">
            <label>Categoría de Comisión</label>
            <select value={catCom} onChange={e => setCatCom(e.target.value)}>
              <option value="">— Sin comisión —</option>
              {CATEGORIAS_COMISION.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
          <div className="field">
      ...
      {tab === 'catalogo' && (
        <div className="card">
          ...
          <div style={{ overflowX: 'auto' }}>
            <table className="tbl">
              <thead><tr>
                <th>Código</th><th>Producto</th><th>Categoría</th><th>Cat. Comisión</th><th>Unidad</th>
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td style={{ fontSize: 11 }}>
                        {p.categoria_comision
                          ? <span className="badge badge-teal">{p.categoria_comision}</span>
                          : <span className="badge badge-amber">Sin asignar</span>}
                      </td>
