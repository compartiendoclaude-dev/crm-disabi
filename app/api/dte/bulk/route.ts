...
export async function POST(req: NextRequest) {
  ...
  // La importación masiva por ZIP es parte del módulo DTE completo — no del
  // permiso angosto de Ventas (esa solo cubre subir un DTE suelto y
  // vincularlo a su propia venta, ver /api/dte). Se descubrió al revisar esta
  // ruta que no tenía ningún chequeo de permiso — cualquier usuario
  // autenticado podía importar en masa sin importar su rol.
  if (!(await requirePermisoEscritura(guard.user.id, 'dte')))
