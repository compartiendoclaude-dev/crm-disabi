...
export async function POST(req: NextRequest) {
  ...
  if (!(await requirePermisoEscritura(user.id, 'planilla')))
    return NextResponse.json({ error: 'Tu rol no tiene permiso para modificar Planilla' }, { status: 403 })
  ...
  try {
    ...
    if (action === 'save_empleado') {
      const { editId, nombre, cargo, departamento, salario_base, fecha_ingreso, dui, nit, nup_isss, nup_afp, afp, tipo_contrato, activo } = body
      ...
      const obj = {
        ...
        tipo_contrato: tipo_contrato === 'honorarios' ? 'honorarios' : 'empleado',
        activo: activo !== false,
      }
    ...
    if (action === 'save_planilla_registro') {
      ...
      const bloqueoPlanilla = await bloqueadoPorCierre(periodo)
      if (bloqueoPlanilla) return NextResponse.json({ error: bloqueoPlanilla }, { status: 409 })
      ...
      // ══════════════════════════════════════════════════════════════════
      // CONTABILIDAD (Fase 2) — devengo de planilla. Se acumula al cierre del
      // mes del período (finDeMes), no a un día específico dentro de él. En
      // edición se resincroniza: se borra el asiento de devengo anterior y se
      // postea de nuevo con los valores actuales. El pago (pagar_planilla) va
      // en un asiento aparte, con origen propio (el gasto puente que ya se
      // registra en disabi_gastos), para no pisarse con este al resincronizar.
      // ══════════════════════════════════════════════════════════════════
      const { data: empDevengo } = await sb.from('disabi_empleados').select('nombre').eq('id', empleado_id).single()
      if (editId) await borrarAsientoDeOrigen(sb, 'disabi_planilla', registroId)
      await crearAsientoContable(sb, {
        fecha: finDeMes(periodo),
        concepto: `Planilla ${periodo} — ${empDevengo?.nombre ?? empleado_id}`,
        origenTabla: 'disabi_planilla',
        origenId: registroId,
        lineas: [
          { cuenta: CUENTA.SUELDOS_SALARIOS, debe: parseFloat((bruto + bonosMonto).toFixed(2)), descripcion: 'Sueldo devengado' },
          { cuenta: CUENTA.CUOTAS_PATRONALES, debe: parseFloat((isssPat + afpPat).toFixed(2)), descripcion: 'Cuotas patronales' },
          { cuenta: CUENTA.PLANILLA_POR_PAGAR, haber: salarioNeto, descripcion: 'Neto a pagar' },
          { cuenta: CUENTA.RETENCIONES_CUOTAS_PATRONALES, haber: parseFloat((totalDeducc + isssPat + afpPat).toFixed(2)), descripcion: 'Retenciones y cuotas patronales por pagar' },
        ],
        creadoPor: user.id,
      })

      return NextResponse.json({ ok: true, id: registroId, calculo: { isssEmp, afpEmp, renta, totalDeducc, salarioNeto, isssPat, afpPat, costoEmpresa } })
    }
    ...
    if (action === 'generar_planilla_mes') {
      ...
      const bloqueoGenMes = await bloqueadoPorCierre(periodo)
      if (bloqueoGenMes) return NextResponse.json({ error: bloqueoGenMes }, { status: 409 })
      ...
      const { data: registrosGuardados, error } = await sb.from('disabi_planilla')
        .upsert(registros, { onConflict: 'empleado_id,periodo', ignoreDuplicates: false })
        .select()
      if (error) throw error

      // Contabilidad (Fase 2): un asiento de devengo por empleado, resincronizado
      // (borra + repostea) por si se está regenerando un período ya generado antes.
      const nombrePorEmpleado = new Map(empleados.map(e => [e.id, e.nombre]))
      const fechaAsientoMes = finDeMes(periodo)
      for (const reg of registrosGuardados ?? []) {
        await borrarAsientoDeOrigen(sb, 'disabi_planilla', reg.id)
        await crearAsientoContable(sb, {
          fecha: fechaAsientoMes,
          concepto: `Planilla ${periodo} — ${nombrePorEmpleado.get(reg.empleado_id) ?? reg.empleado_id}`,
          origenTabla: 'disabi_planilla',
          origenId: reg.id,
          lineas: [
            { cuenta: CUENTA.SUELDOS_SALARIOS, debe: reg.salario_bruto, descripcion: 'Sueldo devengado' },
            { cuenta: CUENTA.CUOTAS_PATRONALES, debe: parseFloat(((reg.isss_patronal ?? 0) + (reg.afp_patronal ?? 0)).toFixed(2)), descripcion: 'Cuotas patronales' },
            { cuenta: CUENTA.PLANILLA_POR_PAGAR, haber: reg.salario_neto, descripcion: 'Neto a pagar' },
            { cuenta: CUENTA.RETENCIONES_CUOTAS_PATRONALES, haber: parseFloat(((reg.total_deducciones ?? 0) + (reg.isss_patronal ?? 0) + (reg.afp_patronal ?? 0)).toFixed(2)), descripcion: 'Retenciones y cuotas patronales por pagar' },
          ],
          creadoPor: user.id,
        })
      }
    ...
    if (action === 'pagar_planilla') {
      ...
      const bloqueoPagoPlanilla = await bloqueadoPorCierre(fp)
      if (bloqueoPagoPlanilla) return NextResponse.json({ error: bloqueoPagoPlanilla }, { status: 409 })
      ...
      const { data: gastoPlanilla } = await sb.from('disabi_gastos').insert([{
        fecha:       fp,
      ...
      // Contabilidad (Fase 2): sale de Bancos lo que realmente se le deposita
      // al empleado (el neto, ya reflejado en 2105 desde el devengo) — no el
      // costo total empresa (ese ya incluye cuotas patronales que no se le
      // pagan a él). origenTabla es el gasto puente (no la planilla) para no
      // pisarse con el asiento de devengo al resincronizar una edición.
      if (gastoPlanilla) {
        await crearAsientoContable(sb, {
          fecha: fp,
          concepto: `Pago planilla ${reg.periodo} — ${empNombre}`,
          origenTabla: 'disabi_gastos',
          origenId: gastoPlanilla.id,
          lineas: [
            { cuenta: CUENTA.PLANILLA_POR_PAGAR, debe: reg.salario_neto, descripcion: 'Pago de neto a empleado' },
            { cuenta: CUENTA.BANCOS, haber: reg.salario_neto, descripcion: 'Pago desde bancos' },
          ],
          creadoPor: user.id,
        })
      }
    ...
    if (action === 'pagar_planilla_mes') {
      ...
      const bloqueoPagoMes = await bloqueadoPorCierre(fp)
      if (bloqueoPagoMes) return NextResponse.json({ error: bloqueoPagoMes }, { status: 409 })

      // Leer todos los pendientes del período
      const { data: pendientes } = await sb.from('disabi_planilla')
        .select('id, costo_total_empresa, salario_neto, tipo_pago, empleado:disabi_empleados(nombre)')
      ...
      const { data: gastosCreados } = await sb.from('disabi_gastos').insert(gastosInsert).select()

      // Contabilidad (Fase 2): un asiento de pago por empleado — mismo criterio
      // que pagar_planilla (sale de Bancos el neto, no el costo total empresa).
      // Se relacionan gasto↔planilla por posición: gastosInsert se construyó
      // recorriendo `pendientes` en el mismo orden, y Postgres/PostgREST
      // conserva el orden de un insert por lote.
      for (let i = 0; i < (gastosCreados ?? []).length; i++) {
        const gasto = gastosCreados![i]
        const p = pendientes[i]
        const empNombreMes = (p.empleado as { nombre?: string } | null)?.nombre ?? 'Empleado'
        await crearAsientoContable(sb, {
          fecha: fp,
          concepto: `Pago planilla ${periodo} — ${empNombreMes}`,
          origenTabla: 'disabi_gastos',
          origenId: gasto.id,
          lineas: [
            { cuenta: CUENTA.PLANILLA_POR_PAGAR, debe: p.salario_neto, descripcion: 'Pago de neto a empleado' },
            { cuenta: CUENTA.BANCOS, haber: p.salario_neto, descripcion: 'Pago desde bancos' },
          ],
          creadoPor: user.id,
        })
      }
