...
import { estaVencida, today } from '@/lib/utils'
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [
      ...
      // fecha_venta (no fecha_emision) — no tiene numero, saldo ni referencia.
      sb.from('disabi_cxc')
        .select('id, monto_total, monto_pagado, monto_pendiente, fecha_venta, fecha_vence, estado, notas, abonos:disabi_cxc_abonos(*)')
        .ilike('cliente', nombre)
    ...
    const saldoPendiente   = cxcs.filter(c => c.estado !== 'Pagado').reduce((a, c) => a + (c.monto_pendiente ?? 0), 0)
    const totalDevuelto    = devs.reduce((a, d) => a + (d.monto_devuelto ?? 0), 0)
    // `estado` nunca se escribe como 'Vencido' (ver estaVencida en lib/utils.ts) —
    // se deriva por fecha, igual que en dashboard/finanzas.
    const hoy              = today()
    const cxcVencidas      = cxcs.filter(c => estaVencida(c, hoy))
    const montoVencido     = cxcVencidas.reduce((a, c) => a + (c.monto_pendiente ?? 0), 0)
    ...
    for (const cxc of cxcs) {
      ...
      for (const ab of ((cxc as any).abonos ?? [])) {
        lineas.push({
          fecha:       ab.fecha,
          tipo:        'abono',
          descripcion: `Abono a CxC ${cxc.id.slice(0, 8)}`,
          cargo:       0,
          abono:       ab.monto,
          saldo_movimiento: ab.monto,
          referencia:  cxc.id.slice(0, 8),
