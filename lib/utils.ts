...
// ─── ¿Está vencida una CxC/CPP? ───────────────────────────────────────────────
// El campo `estado` de disabi_cxc/disabi_cpp NUNCA se pone en 'Vencido' en
// ningún flujo de escritura del sistema — nace 'Pendiente' y con abonos/pagos
// pasa a 'Parcial' o 'Pagado', pero nada lo transiciona a 'Vencido'. Por eso
// "vencida" se calcula SIEMPRE por fecha (igual que ya hacían las tablas de
// Finanzas al pintar la fila en rojo) y nunca comparando estado === 'Vencido'
// — ese valor existe en el tipo pero ninguna ruta de la API lo escribe.
export function estaVencida(
  row: { fecha_vence?: string | null; estado?: string | null; monto_pendiente?: number | null },
  hoy: string = today()
): boolean {
  if (!row.fecha_vence) return false
  if (row.estado === 'Pagado') return false
  if ((row.monto_pendiente ?? 0) <= 0) return false
  return row.fecha_vence < hoy
}
