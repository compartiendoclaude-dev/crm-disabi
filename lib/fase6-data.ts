...
export async function getProyeccionesData() {
  ...
  const [
    ...
    // disabi_cxc / disabi_cpp no tienen columna 'saldo' — es 'monto_pendiente'
    sb.from('disabi_cxc').select('monto_pendiente, fecha_vence').in('estado', ['Pendiente', 'Parcial', 'Vencido']),
    sb.from('disabi_cpp').select('monto_pendiente, fecha_vence').in('estado', ['Pendiente', 'Parcial']),
  ])
  ...
  const cxcProx = (cxcProxMes ?? []).filter(x => x.fecha_vence >= proxIni && x.fecha_vence <= proxFin).reduce((a, x) => a + x.monto_pendiente, 0)
  const cppProx = (cppProxMes ?? []).filter(x => x.fecha_vence >= proxIni && x.fecha_vence <= proxFin).reduce((a, x) => a + x.monto_pendiente, 0)
